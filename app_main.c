...
uint32_t idCount[15];           // counter for each sensor received data (max 15 Devices)
...
/* ***********************************************
   ...
   ***********************************************/
static void csi_rx_cb(const uint8_t *mac, const uint8_t *incoming, int len) {
...
*/
    if (idCount[myMsg.idSens-1] < myMsg.sCount) {
        ...
        ets_printf("CSI_DATA: %u, %u, %u, %u, %u, %u, %u, %u, %d, %d, ", gCount,myMsg.idSens,myMsg.sCount,idCount[myMsg.idSens-1], myMsg.bc1,myMsg.bc2,bcM1.bcCount,myMsg.retry,myMsg.RSSI1, myMsg.RSSI2);
        
        ets_printf("%d.%02u, %d.%02u, %d.%02u, %d.%02u, %d.%02u, %d.%02u, \"[%d", myMsg.tstamp1/1000, abs(myMsg.tstamp1)%1000, myMsg.tstamp2/1000, abs(myMsg.tstamp2)%1000, myMsg.tstampS/1000, abs(myMsg.tstampS)%1000, t2start/1000, abs(t2start)%1000, tdiff/1000, abs(tdiff)%1000, t2bc/1000, abs(t2bc)%1000, myMsg.data[0]);
        //, t2start, tdiff, t2bc, myMsg.data[0]);
        // first(CSI_ to avoid the first comma [,...
        lt2start = t2start;

        //16jan  i=2; i<len...
        for (int i = 1; i < 216; i++) ets_printf(",%d", myMsg.data[i]);
        ets_printf("]\"\n\r\n");
...
#define DELTA  10     //  50 ms TDMA interval
#define CYCLE  75    //  500 ms                NO DEBUG prints
...
//#define CYCLE  100    //  600 ms                  with DEBUG prints
    ...
    for (uint8_t i=0; i<15;i++) idCount[i]=0;
